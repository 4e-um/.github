...
이 시스템은 “단순 로그 저장”이 아니라, 실시간 사용량 집계 + 임계치(50/80/100%) 초과 감지 + 알림 이벤트 발행을 목표로 합니다.
특히, 100만 회선(subscription) 규모에서 “초당 수십만 이벤트”가 유입되는 최악의 상황을 가정하고 설계했습니다.
...
- Lua 스크립트로 모든 단계를 Redis 내부에서 한 번에 처리
  -> 집계 정확성과 알림 중복 방지를 구조적으로 보장
...
|------|----------|--------------|
| 작업 도중 장애 발생 시 | 처음부터 재실행 | **실패 지점부터 재시작 가능** |
| 실행 이력 및 상태 | 수동 관리 | Job / Step 메타데이터 자동 관리 |
| 모니터링 | 별도 구축 필요 | 운영 환경에 적합한 로그 및 모니터링 제공 |
...
월별 청구내역을 스냅샷 형태로 생성하여 정산 및 조회 시 **일관성과 성능을 보장**하는 배치 시스템입니다.

**주요 기능:**
- 회선별 원천 데이터 조회
- 다중 테이블 JOIN 조회
- 청구내역 Snapshot Entity 생성
- Snapshot Repository 저장
...
- 원천 데이터를 직접 조회하지 않고 **Snapshot 기준으로 후속 배치 수행**
- 정산 배치와 책임 분리
- 재시작 시에도 동일한 기준 데이터 보장
...
#### 5.8.1. 재시작 전략 고도화
- StepExecutionContext 기반 재시작 조건 명확화
- 부분 실패 시 안전한 복구 전략 적용

#### 5.8.2. S3 연동 및 정합성 전략
- PostgreSQL 저장 Step → AfterStep Listener로 이동
- S3 저장 로직을 이전 Step으로 분리
- S3 실패 시 메인 DB와 정합성 보장
...
#### 5.8.3. 테스트 전략
- 단위 테스트: 계산 로직, Snapshot 생성 검증
- 통합 테스트: Job 재시작, Partition 동작 검증
